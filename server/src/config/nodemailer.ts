...
export const sendSubAdminWelcomeEmail = async (officer: {
  name: string;
  email: string;
  rawPassword?: string;
  department: string;
  ...
  officialId: string;
}): Promise<{ success: boolean; devMode?: boolean; messageId?: string; error?: string }> => {
  const emailUser = (process.env.EMAIL_USER || '').trim();
  const emailPass = (process.env.EMAIL_PASS || '').replace(/\s+/g, '');
    ...
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 24px; border: 1px solid #e2e8f0; border-radius: 12px; background-color: #ffffff;">
      ...
      <div style="background-color: #f8fafc; padding: 20px; border-radius: 8px; margin-bottom: 20px;">
        ...
        <table style="width: 100%; font-size: 14px; color: #334155;">
          ...
          <tr><td style="padding: 6px 0; font-weight: 600;">Officer ID:</td><td style="padding: 6px 0; font-weight: 700; font-family: monospace;">${officer.officialId}</td></tr>
      ...
      <div style="text-align: center; margin-bottom: 20px;">
        <a href="${loginUrl}" style="display: inline-block; padding: 12px 32px; background-color: #0284c7; color: #ffffff; text-decoration: none; font-weight: 700; font-size: 14px; border-radius: 8px;">Login to Admin Portal</a>
      ...
      <div style="background-color: #fffbeb; padding: 12px 16px; border-radius: 8px; border: 1px solid #fde68a; margin-bottom: 16px;">
        <p style="color: #92400e; font-size: 12px; margin: 0;"><strong>⚠ Security Notice:</strong> Please log in to the District Admin Portal and update your password. Keep these credentials safe.</p>
      </div>

      <p style="color: #94a3b8; font-size: 11px; text-align: center; margin: 0;">
        Automated message from CivicLens State Governance System.
      </p>
    </div>
  `;
